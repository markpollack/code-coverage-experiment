...
The agent is Claude Sonnet 4.6 invoked via the Claude Code SDK. Each invocation
is a self-contained session: the agent reads the project, writes tests, compiles,
runs, checks coverage, and iterates until satisfied or the timeout expires.
...
Each agent invocation is captured via the Claude Code SDK session log.
The captured exhaust includes:
...
For two-phase variant-d, both phases run in the same Claude Code session ---
the explore phase's context is preserved for the act phase. Each phase's
exhaust is captured separately.
